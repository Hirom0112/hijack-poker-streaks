...
|---|---|
| **DynamoDB unavailable** | Repository surfaces the SDK error; handler maps to `503` `{error:"unavailable"}`. No partial state written (writes are conditional/transactional). Reads fail closed (no stale fabrication). Logged with `playerId`+`correlationId` (§8). |
