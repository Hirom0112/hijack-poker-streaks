...
- [x] S0-1: Add backend TS toolchain to `streaks-api/package.json` — `typescript ^5.4.0`, `ts-jest ^29.1.0`, `serverless-esbuild ^0.8.0` (dev), `luxon ^3.4.0` (prod), `@types/luxon ^3.4.0` (dev); add `"typecheck": "tsc --noEmit"`; switch `test` to ts-jest; set `jest.testMatch` to `**/__tests__/**/*.test.ts`. *(check: `npm install` succeeds; exactly 5 new installs)*
