...
| **TS build / bundle for Lambda** | **esbuild via `serverless-esbuild`** | `serverless-esbuild ^0.8.0` (NEW backend) — **ASSUMPTION:** version (latest 0.8.x for Serverless v3) | *Plain `tsc` then deploy the JS tree.* Rejected: `serverless-esbuild` bundles + transpiles TS per-function at `sls package`/`offline` time, tree-shakes, and is the de-facto TS path for Serverless v3 — keeps `serverless-offline` working with `.ts` handlers and produces a small Lambda artifact. `tsc`-only would ship `node_modules` and need a separate build step glued into the serverless lifecycle. |
...
| `serverless-esbuild ^0.8.0` | dev | Bundle/transpile `.ts` handlers for `serverless offline` + `sls package` on Serverless v3. |
...
| `INTERNAL_API_SECRET` **(ADD)** | Shared secret guarding the internal `POST /internal/streaks/hand-completed` endpoint (FR-6.3, NFR-3) **and reused by the admin view-history endpoint (FR-8.2)** — no new secret needed | `local-internal-secret` (**ASSUMPTION:** dev default) | **ADD** to docker-compose `streaks-api` env + `.env.example`; checked by the internal-auth middleware |
